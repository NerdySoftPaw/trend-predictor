...
The integration watches a sensor (e.g. battery state of charge in percent), analyses its history over a configurable time window and uses linear regression to calculate when the sensor will reach a defined target value.

## The three sensors

| Sensor | Example value | Description |
|--------|--------------|-------------|
| **Time remaining** | `4.2 h` | Hours until the target value is reached |
...
All three sensors show `unavailable` when the trend is moving away from the target or there are not enough data points yet.
